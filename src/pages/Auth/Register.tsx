...
const Register: React.FC = () => {
  ...
  const [validationErrors, setValidationErrors] = useState<{ email?: string; password?: string; confirmPassword?: string; fullName?: string }>({});
  const [showSuccessModal, setShowSuccessModal] = useState(false);
  const { signUp, signInWithGoogle, signInWithGitHub, loading, error, user } = useAuthStore();
      ...
      <div>
        <div className="flex items-center mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-white/70">设置您的账号信息</p>
          </div>
        </div>
        ...
        <div className="mt-8">
          <div className="flex items-center gap-3">
            ...
            <p className="text-xs text-white/70">或使用以下方式登录</p>
            <div className="flex-1 h-px bg-white/20"></div>
          </div>
        ...
        {showSuccessModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-gray-900/95 backdrop-blur-xl rounded-2xl shadow-2xl w-full max-w-md border border-white/10 overflow-hidden">
              <div className="p-8 text-center">
                ...
                <p className="text-sm text-white/70 mb-6">您的账号已创建成功，请登录</p>
                <Button
                  variant="primary"
                  fullWidth
